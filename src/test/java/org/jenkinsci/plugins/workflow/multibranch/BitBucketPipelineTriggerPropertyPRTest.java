...
public class BitBucketPipelineTriggerPropertyPRTest {

    @Rule
    public JenkinsRule jenkins = new JenkinsRule();

    private final List<String> branchNames = Collections.singletonList("master");
    private final List<String> prNames = Collections.singletonList("PR-1");
    private final int expectedPipelineCount = this.branchNames.size() + this.prNames.size();
    private final String branchIncludeFilter;
    private final String branchExcludeFilter;
    private final String username = "aytuncbeken-test";
    ...
    public BitBucketPipelineTriggerPropertyPRTest(String branchIncludeFilter, String branchExcludeFilter) {
    ...
    public void testPR() throws Exception {
        if( System.getProperty("bitbucket.password") == null) {
            System.out.println("BitBucket Password not set, skipping test");
            return;
        }
        String password = System.getProperty("bitbucket.password");
        credentials = new UsernamePasswordCredentialsImpl(CredentialsScope.GLOBAL, UUID.randomUUID().toString(),"",this.username, password);
    ...
    private WorkflowMultiBranchProject createWorkflowMultiBranchJobWithTriggers(
        ...
        BitbucketSCMSource bitbucketSCMSource = new BitbucketSCMSource(this.username,this.repository);
        bitbucketSCMSource.setCredentialsId(credentials.getId());
        bitbucketSCMSource.setTraits(Arrays.asList(new BranchDiscoveryTrait(true,false), new WildcardSCMHeadFilterTrait("*",""), new OriginPullRequestDiscoveryTrait(1)));
        workflowMultiBranchProject.setSourcesList(Arrays.asList(new BranchSource(bitbucketSCMSource)));
    ...
    public void checkResults(
        ...
        workflowMultiBranchProject.getSourcesList().add(new BranchSource(bitbucketSCMSource));
